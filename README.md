Python
# Generating the README.md content for the user's GitHub repository
readme_content = """# 🔍 Threat Hunting & Anomaly Detection com Splunk (SPL)
...
* **Fontes de Dados (Log Sources):** * Eventos do Windows (EventID 4624 - Logins Bem-sucedidos)
  * Logs do Sysmon (Microsoft-Windows-Sysmon/Operational)
...
A análise inicial consistiu em mapear o comportamento padrão (baseline) do ambiente computacional do Windows. Através de queries focadas em IDs específicos e ordenações estatísticas, isolei os executáveis dominantes no ecossistema de rede.
...
Filtro de Conexões e Movimentação Lateral:
Análise de conexões direcionadas a portas críticas de gerencimento do Windows (como a porta RPC 135) originadas por endereços IPs externos.
...
Campo Image (Caminho do Binário)
RiskScore
Significado Analítico
C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe
10
Crítico: Indica potencial abuso de LOLBins (Living off the Land Binaries) para execução de scripts arbitrários em memória.
C:\\Windows\\System32\\lsass.exe
9
Alto: Alvo comum para técnicas de Credential Dumping (ex: Mimikatz).
